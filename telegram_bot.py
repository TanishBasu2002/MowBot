...
#       for selection (using job IDs so that selection appears green), then an "Assign Selected"
#       button to assign them to either Andy or Alex.
#     • "View Completed Jobs" – shows a sub‑menu with "Andy" and "Alex" buttons;
#       choosing one lists that employee’s completed jobs as buttons. Tapping a job shows
#       the job’s details, photos, etc.
...
# Modified to handle job tuples with either 7 or 8 fields.
async def format_job_section(section_title: str, jobs: list) -> list:
    # We assume the status is the fourth field if 8 fields, or third if 7 fields.
    if len(jobs[0]) == 8:
        status_val = jobs[0][5]
    else:
        status_val = jobs[0][3]
...
async def director_calendar_view(update: Update, context: CallbackContext):
    # Show sub-menu with two buttons for Andy and Alex.
...
async def director_calendar_view(update: Update, context: CallbackContext):
    # Show sub-menu for completed jobs
    kb = InlineKeyboardMarkup([
         [InlineKeyboardButton("Andy", callback_data="view_completed_jobs_1672989849")],
         [InlineKeyboardButton("Alex", callback_data="view_completed_jobs_777888999")],
         [InlineKeyboardButton(f"{ButtonLayouts.BACK_PREFIX} Back", callback_data="director_dashboard")]
